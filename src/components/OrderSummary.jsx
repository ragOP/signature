...
const OrderSummary = ({ subtotal, discount, total, isCheckingOut, onCheckout }) => {
  ...
  // Reset showMinLoader when isCheckingOut becomes true
  useEffect(() => {
    if (isCheckingOut) {
  ...
  const handleButtonClick = async () => {
    if (isCheckingOut || showMinLoader) return;
      ...
      <div className="relative group">
        ...
        <div className="relative bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl rounded-3xl p-6 border border-white/20">
          ...
          <button
            ...
            disabled={isCheckingOut || showMinLoader}
            className={`w-full bg-gradient-to-r from-red-500 to-pink-500 text-white py-4 rounded-full font-semibold transition-all duration-300 transform shadow-xl buy-now-shimmer disabled:opacity-50 disabled:cursor-not-allowed ${isPressed
